...
describe('_timelineEvent-GOAL_UPDATED', () => {
  it('should display GOAL_UPDATED timeline event', () => {
    // Given
    ...
    // When
    const content = nunjucks.render('_timelineEvent-GOAL_UPDATED.njk', model)
    const $ = cheerio.load(content)

    // Then
    expect($('[data-qa-event-type=GOAL_UPDATED]').length).toEqual(1)
    expect($('.moj-timeline__title').text().trim()).toEqual('Goal updated')
  ...
  it('should display GOAL_UPDATED timeline event given prison name not looked up', () => {
    // Given
    const prisonerSummary = aValidPrisonerSummary()
    const event = aTimelineEvent({
      eventType: 'GOAL_UPDATED',
