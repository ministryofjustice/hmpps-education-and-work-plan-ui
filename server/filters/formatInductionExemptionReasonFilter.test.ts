  ...
  describe('should format view model values', () => {
    Array.of(
      ...
      },
      {
        ...
        expected: 'Induction logged late due to technical issue with learning and work progress service',
      },
      {
        source: InductionScheduleStatusValue.EXEMPT_SCREENING_AND_ASSESSMENT_IN_PROGRESS,
        expected:
          'Screening and assessment in progress for an identified learning need, neurodivergence or health concern',
      },
      {
        source: InductionScheduleStatusValue.EXEMPT_SCREENING_AND_ASSESSMENT_INCOMPLETE,
        expected:
          'Cannot complete session to a reasonable quality without information from the screening and assessment',
      },
    ).forEach(spec => {
