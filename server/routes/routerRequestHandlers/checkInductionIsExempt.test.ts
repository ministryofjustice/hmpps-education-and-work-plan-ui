...
import checkInductionIsExempt from './checkInductionIsExempt'
...
describe('checkInductionIsExempt', () => {
  ...
  it.each([
    ...
    'should call next with no arguments given prisoners induction schedule is one of the supported exemptions',
  ...
  it.each([
    ...
    'should call next with an error given prisoners induction schedule is not one of the supported exemptions',
    async scheduleStatus => {
      // Given
      const inductionSchedule = aValidInductionSchedule({ scheduleStatus })
      inductionService.getInductionSchedule.mockResolvedValue(inductionSchedule)
      ...
      const expectedError = createError(
        404,
        'Induction Schedule for prisoner A1234BC is not one of the Exemption reasons',
      )
